...
class HelperTest(unittest.TestCase):
    ...
    def test_geometry_names_each_unmeasured_section_and_why(self):
        ...
        hidden, hidden_list, missing, unknown, excluded = in_scope[1:6]
        ...
        def drop_boxes(raw):
            for key in (hidden, hidden_list, missing, unknown, excluded):
                raw["boxes"].pop(key)
            raw["targets"][unknown].pop("visible")
        ...
        self.assertIn(f"no geometry for {unknown} (visibility not recorded; re-run the capture script)",
                      result.stdout)
